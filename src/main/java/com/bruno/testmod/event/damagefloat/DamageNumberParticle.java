...
//                    DF(1).format(number), DF(1).format(critMult));
//        }
    ...
    public void render(VertexConsumer pBuffer, Camera pCamera, float pPartialTicks) {
        ...
        float particleX = (float) (Mth.lerp(pPartialTicks, this.xo, this.x) - cameraPos.x());
        float particleY = (float) (Mth.lerp(pPartialTicks, this.yo, this.y) - cameraPos.y());
        float particleZ = (float) (Mth.lerp(pPartialTicks, this.zo, this.z) - cameraPos.z());


        //int light = ClientConfigs.LIT_UP_PARTICLES.get() ? LightTexture.FULL_BRIGHT : this.getLightColor(pPartialTicks);
        ...
        // animation
        poseStack.translate(0, (1 + inc / 4f) * Mth.lerp(pPartialTicks, this.prevVisualDY, this.visualDY), 0);
        // rotate towards camera

        float fadeout = Mth.lerp(pPartialTicks, this.prevFadeout, this.fadeout);

        float defScale = 0.006f;
        float scale = (float) (defScale * distanceFromCam);
        poseStack.mulPose(pCamera.rotation());

        // animation
        poseStack.translate((1 + inc) * Mth.lerp(pPartialTicks, this.prevVisualDX, this.visualDX), 0, 0);
